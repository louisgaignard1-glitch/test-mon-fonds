"""
factsheet_export.py  —  v6
---------------------------
Améliorations v6 :
  • g_perf     : axes X (années) + Y (%) + légende + grille complète
  • g_dd       : axes X/Y + annotation max drawdown + légende
  • g_monthly  : stacké par classe d'actif (Actions EU / Actions US / Fonds)
                  avec légende couleur claire — on sait QUI contribue
  • g_pie      : donut repensé — palette distincte par classe, pas de
                  autopct illisible, légende externe propre avec %
  • Toutes les figures : titre, sous-titre, axes nommés, grille Y
"""
...
#  PALETTE
# ═══════════════════════════════════════════════════════════════
P_NAVY   = colors.HexColor("#1B3A6B")
P_MID    = colors.HexColor("#2E5FA3")
P_GOLD   = colors.HexColor("#B8972A")
P_GOLD_L = colors.HexColor("#FBF3DF")
P_BLUE_L = colors.HexColor("#EDF2FB")
P_STRIPE = colors.HexColor("#F5F8FC")
P_BORDER = colors.HexColor("#C8D4E8")
P_TEXT   = colors.HexColor("#1A2E4A")
P_MUTED  = colors.HexColor("#607080")
...
# Couleurs par classe d'actif — cohérentes partout (donut + barres)
CLS_COLORS = {
    "Actions EU": "#2E5FA3",
    "Actions US": "#E07B39",
    "Fonds":      "#4A9B7F",
...
def compute_metrics(port_idx, bench_idx, port_ret):
    r = port_ret.dropna(); ann = 252; rf = 0.035 / ann

    def _p(s, d):
        cut = s.index[-1] - pd.Timedelta(days=d)
        sub = s[s.index >= cut]
        return (sub.iloc[-1] / sub.iloc[0] - 1) * 100 if len(sub) > 1 else np.nan

    ytd0 = pd.Timestamp(datetime.date.today().year, 1, 1)
    def _ytd(s):
        sub = s[s.index >= ytd0]
        return (sub.iloc[-1] / sub.iloc[0] - 1) * 100 if len(sub) > 1 else np.nan
...
# ═══════════════════════════════════════════════════════════════
#  STYLE MATPLOTLIB COMMUN
# ═══════════════════════════════════════════════════════════════
_RC = {
    "figure.facecolor": "white",  "axes.facecolor": "white",
    "axes.spines.top":  False,    "axes.spines.right": False,
    "axes.spines.left": True,     "axes.spines.bottom": True,
    "axes.edgecolor": M_BDR,      "axes.linewidth": 0.8,
    "xtick.color": M_GRAY,        "ytick.color": M_GRAY,
    "xtick.labelsize": 6.5,       "ytick.labelsize": 6.5,
    "xtick.major.size": 3,        "ytick.major.size": 3,
    "xtick.major.pad": 3,         "ytick.major.pad": 3,
    "grid.color": "#E8EEF6",      "grid.linewidth": 0.45,
    "axes.titlesize": 8,          "axes.titlecolor": M_NAVY,
    "axes.titleweight": "bold",   "axes.titlelocation": "left",
    "axes.titlepad": 6,
    "legend.fontsize": 6.5,       "legend.framealpha": 0.92,
    "legend.edgecolor": M_BDR,    "legend.fancybox": False,
    "legend.borderpad": 0.5,      "legend.labelspacing": 0.3,
}
...
def g_perf(port_idx, bench_idx, w_pt, h_pt):
    with plt.rc_context(_RC):
        ...
        ax.fill_between(p.index, p, 0, alpha=0.08, color=M_NAVY, zorder=1)
        l1, = ax.plot(p.index, p, color=M_NAVY, lw=1.8, zorder=3,
                      label="Portefeuille hedgé USD")
        l2, = ax.plot(b.index, b, color=M_GOLD, lw=1.2, ls="--",
                      dashes=(5, 2.5), zorder=2, label="Benchmark composite")
        ax.axhline(0, color=M_BDR, lw=0.7, ls=":", zorder=0)

        ax.set_xlabel("Année", fontsize=6.5, color=M_GRAY, labelpad=4)
        ax.set_ylabel("Performance cumulée (%)", fontsize=6.5, color=M_GRAY, labelpad=4)
        ax.xaxis.set_major_locator(mdates.YearLocator())
        ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
        ax.yaxis.set_major_formatter(
            mticker.FuncFormatter(lambda x, _: f"{x:+.0f}%"))
        ax.grid(True, axis="both", zorder=0, alpha=0.6)
        ax.legend(loc="upper left", handlelength=1.8)
        ax.set_title("Performance cumulée nette de frais  ·  base 0 % au départ")
        fig.subplots_adjust(left=0.09, right=0.99, top=0.88, bottom=0.14)
        return _png(fig)
...
def g_dd(port_ret, w_pt, h_pt):
    with plt.rc_context(_RC):
        fig, ax = plt.subplots(figsize=(max(w_pt/72, 2.5), max(h_pt/72, 1.8)))
        cum = (1 + port_ret.dropna()).cumprod()
        dd  = ((cum - cum.cummax()) / cum.cummax()) * 100

        ax.fill_between(dd.index, dd, 0, color=M_RED, alpha=0.22, label="Drawdown")
        ax.plot(dd.index, dd, color=M_RED, lw=1.0)
        ...
        ax.annotate(
            ...
            fontsize=5.5, color=M_RED,
            arrowprops=dict(arrowstyle="-|>", color=M_RED,
                            lw=0.7, mutation_scale=6),
            ha="left", va="bottom",
        )
        ax.axhline(0, color=M_BDR, lw=0.6)
        ax.set_xlabel("Année", fontsize=6.5, color=M_GRAY, labelpad=4)
        ax.set_ylabel("Drawdown (%)", fontsize=6.5, color=M_GRAY, labelpad=4)
        ax.xaxis.set_major_locator(mdates.YearLocator())
        ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
        ax.yaxis.set_major_formatter(
            mticker.FuncFormatter(lambda x, _: f"{x:.0f}%"))
        ...
        ax.set_title("Drawdown depuis le sommet  ·  portefeuille hedgé USD")
        fig.subplots_adjust(left=0.12, right=0.99, top=0.88, bottom=0.16)
        return _png(fig)


# ═══════════════════════════════════════════════════════════════
#  GRAPHIQUE 3 — Barres mensuelles EMPILÉES PAR CLASSE D'ACTIF
# ═══════════════════════════════════════════════════════════════
def g_monthly(port_ret, weights, w_pt, h_pt):
    """
    Barres mensuelles empilées par classe d'actif.
    La contribution de chaque classe = perf_mensuelle × fraction_poids_classe.
    C'est une approximation lisible : le vrai Return Attribution
    nécessiterait les prix individuels de chaque ligne.
    """
    today_yr = datetime.date.today().year
    monthly_total = pd.Series(dtype=float)
    chosen = today_yr

    for cand in [today_yr, today_yr - 1, today_yr - 2]:
        sub = port_ret[port_ret.index.year == cand]
        if not sub.empty:
            monthly_total = sub.resample("ME").apply(
        ...
        if monthly_total.empty:
            ...
            for cls in CLS_ORDER:
                ...
                ax.bar(xs, bar_pos, bottom=bottom_pos, color=color,
                       edgecolor="white", lw=0.4, width=0.65, zorder=3, alpha=0.88)
                ax.bar(xs, bar_neg, bottom=bottom_neg, color=color,
                       edgecolor="white", lw=0.4, width=0.65, zorder=3, alpha=0.88)
                bottom_pos += bar_pos
                bottom_neg += bar_neg
                legend_handles.append(
                    Patch(facecolor=color, edgecolor="white", label=cls))

            # Valeur totale sur chaque barre
            ymax = max(abs(monthly_total.values)) if len(monthly_total) else 1
            for xi, val in zip(xs, monthly_total.values):
                off = ymax * 0.08 if val >= 0 else -ymax * 0.08
                va  = "bottom" if val >= 0 else "top"
                ax.text(xi, val + off, f"{val:+.1f}%",
                        ha="center", va=va, fontsize=5.5,
                        color=M_NAVY, fontweight="bold", zorder=5)

            ax.axhline(0, color=M_BDR, lw=0.7, zorder=2)
            ax.set_xticks(xs)
            ax.set_xticklabels(x_labels)
            ax.set_xlim(-0.6, len(monthly_total) - 0.4)
            ax.set_xlabel("Mois", fontsize=6.5, color=M_GRAY, labelpad=4)
            ax.set_ylabel("Contribution (%)", fontsize=6.5, color=M_GRAY, labelpad=4)
            ax.yaxis.set_major_formatter(
                mticker.FuncFormatter(lambda x, _: f"{x:+.1f}%"))
            ax.grid(True, axis="y", zorder=0, alpha=0.6)
            ax.legend(handles=legend_handles, loc="upper right",
                      handlelength=1.1, handleheight=0.9)

        ax.set_title(f"Contributions mensuelles {chosen}  ·  par classe d'actifs")
        fig.subplots_adjust(left=0.11, right=0.99, top=0.88, bottom=0.14)
        return _png(fig)


# ═══════════════════════════════════════════════════════════════
#  GRAPHIQUE 4 — Donut allocation (entièrement repensé)
# ═══════════════════════════════════════════════════════════════
def g_pie(cls_data, side_pt):
    """
    Donut sobre et lisible :
    - Palette cohérente avec CLS_COLORS
    - Pas d'autopct : les % sont dans la légende externe
    - Texte central "Allocation / 100%"
    - Figure rectangulaire : donut gauche, légende droite
    """
    ...
    side_in = max(side_pt / 72, 1.6)
    fig = plt.figure(figsize=(side_in * 1.7, side_in), facecolor="white")

    # Zone donut — 52% de la largeur
    ...
    # Cercle central
    centre_circle = plt.Circle((0, 0), 0.48, fc="white", zorder=10)
    ax.add_patch(centre_circle)
    ax.text(0,  0.10, "Allocation", ha="center", va="center",
            fontsize=5.5, color=M_GRAY)
    ax.text(0, -0.15, "100 %", ha="center", va="center",
            fontsize=8.5, color=M_NAVY, fontweight="bold")

    # Légende externe avec % explicite
    ax_leg = fig.add_axes([0.55, 0.05, 0.43, 0.90])
    ax_leg.axis("off")
    handles = [Patch(facecolor=CLS_COLORS[l], edgecolor="white", linewidth=0.5)
               for l in labels]
    leg_labels = [f"{l}\n{v/total*100:.1f}%" for l, v in zip(labels, values)]
    leg = ax_leg.legend(
        handles, leg_labels,
        loc="center left",
    ...
    for text in leg.get_texts():
        text.set_color(M_NAVY)

    fig.text(0.03, 0.97, "Répartition par classe d'actifs",
             fontsize=7, fontweight="bold", color=M_NAVY,
             va="top", ha="left")
    return _png(fig, dpi=96)


# ═══════════════════════════════════════════════════════════════
#  PRIMITIVES CANVAS
# ═══════════════════════════════════════════════════════════════
def _r(c, x, y, w, h, fill=None, stroke=None, sw=0.3):
    c.saveState()
...
def _t(c, x, y, text, font="Helvetica", size=8, color=P_TEXT, align="left"):
    c.saveState()
    c.setFont(font, size); c.setFillColor(color)
    {"left": c.drawString, "right": c.drawRightString,
     "center": c.drawCentredString}[align](x, y, str(text))
    c.restoreState()
...
def _img_rect(c, png_bytes, x, y, w, h):
    c.drawImage(ImageReader(io.BytesIO(png_bytes)),
                x, y, width=w, height=h, preserveAspectRatio=False)
...
def sec(c, x, y, title, w=None):
    ...
    _r(c, x, y - ST_H, bw, ST_H, fill=P_NAVY)
    _r(c, x, y - ST_H, 3.2*mm, ST_H, fill=P_GOLD)
    _hline(c, x, x + bw, y - ST_H, color=P_GOLD, lw=0.9)
    mid_y = y - ST_H + (ST_H - 8) / 2
    _t(c, x + 5*mm, mid_y, title, font="Helvetica-Bold", size=8, color=P_WHITE)
    return y - ST_H - 2*mm

def thead(c, x, y, cols, widths):
    _r(c, x, y - TH_H, sum(widths), TH_H, fill=P_MID)
    ...
    for i, (col, w) in enumerate(zip(cols, widths)):
        ...
        _t(c, cx + w/2, mid, col, font="Helvetica-Bold", size=6.5,
           color=P_WHITE, align="center")
        cx += w
    return y - TH_H

def trows(c, x, y, rows, widths, aligns=None, rh=TR_H,
          bold_last=False, stripe=True):
    aligns = aligns or ["left"] * len(widths)
    for i, row in enumerate(rows):
        bg = P_STRIPE if (stripe and i % 2 == 0) else P_WHITE
        _r(c, x, y - rh, sum(widths), rh, fill=bg, stroke=P_BORDER, sw=0.2)
        cx = x
        ty = y - rh + (rh - 6.5) / 2
        for j, (cell, w) in enumerate(zip(row, widths)):
            ...
            pad = 2.2 * mm
            if   aligns[j] == "left":  _t(c, cx + pad,     ty, s, fnt, 6.5, col, "left")
            elif aligns[j] == "right": _t(c, cx + w - pad, ty, s, fnt, 6.5, col, "right")
            else:                      _t(c, cx + w/2,      ty, s, fnt, 6.5, col, "center")
            cx += w
        y -= rh
    return y
...
def page1(c, port_idx, bench_idx, port_ret, weights, m, as_of, today):
    ...
    ch = 63 * mm
    _img_rect(c, g_perf(port_idx, bench_idx, CW*(72/mm), ch*(72/mm)),
              ML, y-ch, CW, ch)
    y -= ch + GAP
    ...
    _img_rect(c, g_dd(port_ret, col2*(72/mm), H2*(72/mm)),
              ML, yl-H2, col2, H2)

    x2 = ML + col2 + 5*mm
    yr = sec(c, x2, ytop, "CONTRIBUTIONS MENSUELLES  ·  par classe d'actifs", w=col2)
    _img_rect(c, g_monthly(port_ret, weights, col2*(72/mm), H2*(72/mm)),
              x2, yr-H2, col2, H2)
...
def page2(c, port_ret, allocation, ticker_names, weights, as_of, today):
    ...
    # Ligne total
    _r(c, ML, y-5*mm, sum(cw), 5*mm, fill=P_NAVY)
    mid_tot = y - 5*mm + (5*mm - 7) / 2
    _t(c, ML+3*mm, mid_tot, "TOTAL", font="Helvetica-Bold", size=7, color=P_WHITE)
    _t(c, ML+sum(cw)-3*mm, mid_tot,
    ...
    for k in CLS_ORDER:
        ...
        c.restoreState()
        _t(c, ML + 5.5*mm, ly - 2.2*mm, k, font="Helvetica", size=6, color=P_TEXT)
        ly -= 4*mm
    ...
    # Historique mensuel
    y     = sec(c, ML, y,
                "HISTORIQUE DE PERFORMANCE MENSUELLE  —  PORTEFEUILLE HEDGÉ USD")
    ...
    # Disclaimer
    dz = 18 * mm
    _r(c, 0, FTR_H, W, dz - FTR_H, fill=colors.HexColor("#F4F7FC"))
...
def render_factsheet_section(port_idx, bench_idx, port_ret,
    ...
    with col_btn:
        if st.button("⬇️ Générer la factsheet", type="primary",
